...
- (void) importMessages {
    ...
    for (NSString *chatJID in self.chats) { @autoreleasepool {
        NSManagedObject *chat = [self.chats objectForKey:chatJID];
        NSLog(@"Importing messages for chat: %@", chat);
        ...
        for (NSDictionary *amsg in results) {
            ...
            msg = [NSEntityDescription insertNewObjectForEntityForName:@"ZWAMESSAGE"
                                                inManagedObjectContext:self.moc];
            ...
            // NSDate *timestamp = [self convertAndroidTimestamp:[amsg objectForKey:@"timestamp"]];
            [msg setValue:timestamp forKey:@"ZMESSAGEDATE"]; // Modern: 'timestamp' is likely a double
            // TODO sentDate

            [msg setValue:[NSNumber numberWithBool:fromMe] forKey:@"ZISFROMME"]; // Modern: 'from_me' is likely an integer 0/1
            if (!fromMe) {
                [msg setValue:chatJID forKey:@"ZFROMZID"]; // The JID of the chat partner or group
                if (isGroup) {
                    ...
                    if (member) { // Only set if member is found/created
                        [msg setValue:member forKey:@"ZGROUPMEMBER"]; // The JID of the chat partner or group
                    }
                }
            } else { // Message is from me
                [msg setValue:chatJID forKey:@"ZTOJID"]; // The JID of the chat partner or group
                ...
                [msg setValue:@8 forKey:@"ZMESSAGESTATUS"]; // Modern: 'status' might be used for this
            }

            // Sort will be fixed later, but initialize for now
            [msg setValue:[NSNumber numberWithInt:0] forKey:@"ZSORT"]; // Placeholder, will be updated

            // Modern: 'key_id' for stanzaID
            id stanzaID = [amsg objectForKey:@"key_id"];
            if (stanzaID && ![stanzaID isEqual:null]) {
                [msg setValue:stanzaID forKey:@"ZSTANZAID"]; // Modern: 'key_id' for stanzaID
            }

            [msg setValue:@3 forKey:@"ZDATAITEMVERSION"]; // Assuming this iOS-specific field remains
            ...
            [msg setValue:@(MSG_TEXT) forKey:@"ZMESSAGETYPE"]; // iOS side always gets MSG_TEXT for these placeholders
            ...
            [msg setValue:chat forKey:@"ZCHATSESSION"]; // Link to chat session
        }

        // // Fix sort fields (this logic can remain largely the same)
        // // Fetch existing messages already in CoreData for this chat session
        // NSFetchRequest *fetchRequest = [NSFetchRequest fetchRequestWithEntityName:@"ZWAMESSAGE"];
        // NSPredicate *predicate = [NSPredicate predicateWithFormat:@"ZCHATSESSION = %@", [chat objectID]];
        // NSSortDescriptor *sortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"ZMESSAGEDATE" ascending:YES]; // Sort by date first
        ...
        NSFetchRequest *fetchRequest = [NSFetchRequest fetchRequestWithEntityName:@"ZWAMESSAGE"];
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"ZCHATSESSION = %@", [chat objectID]];
        NSSortDescriptor *sortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"ZSORT" ascending:YES];
        [fetchRequest setSortDescriptors:[NSArray arrayWithObject:sortDescriptor]];
        [fetchRequest setPredicate:predicate];
        ...
        for (msg in newMessages) {
            [msg setValue:[NSNumber numberWithInt:(sort++)] forKey:@"ZSORT"];
        }

        // When new message arrive, it's sort field is taken from chat's counter
        [chat setValue:[NSNumber numberWithInt:sort] forKey:@"ZMESSAGECOUNTER"];

        // Link last message
        if (msg != nil) {
            [chat setValue:msg forKey:@"ZLASTMESSAGE"];
            [chat setValue:[msg valueForKey:@"ZTEXT"] forKey:@"ZLASTMESSAGETEXT"];
            [chat setValue:[msg valueForKey:@"ZMESSAGEDATE"] forKey:@"ZLASTMESSAGEDATE"];
        }
